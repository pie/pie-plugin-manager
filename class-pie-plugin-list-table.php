...
if ( ! class_exists( 'PiePluginListTable' ) ) {
	class PiePluginListTable extends \Wp_List_Table {
		...
		public function __construct( $plugin_paths = [], $plugin_states = [], $theme_path = '' ) {
			parent::__construct([
		...
		public function wp_list_table_data( $orderby ='', $order = '') {
			...
			foreach( $this->plugin_paths as $path ) {
				$plugin_data = get_plugin_data( $this->theme_path . '/plugins/' . $path );
				$data[] = [
					"plugin_name"        => $plugin_data['Name'],
					"plugin-description" => $plugin_data['Description'],
					"plugin-data"        => $plugin_data,
					"plugin_state"       => $this->plugin_states[$path],
					'plugin-slug'        => $path
				];
			}
