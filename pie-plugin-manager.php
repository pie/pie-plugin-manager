...
Description: Allows the user to view and toggle plugins that are in a PIE theme (plugin directory and main file name MUST be the same)
Version:     1.0
...
if ( !class_exists( 'PiePluginManager' ) ) {
  ...
  class PiePluginManager {
    ...
    public function __construct() {
      ...
      $this->theme_path = wp_get_theme()->get_stylesheet_directory();
      if ( is_dir( $this->theme_path . '/plugins' ) ) {
        $this->plugin_states = $this->get_plugin_states( $this->theme_path );
        $this->load_plugins();
        add_action( 'admin_init', [$this, 'update_plugins'] );
    ...
    /**
     * Require all files for active plugins on this page
     */
    public function load_plugins() {
      foreach ( $this->plugin_states as $plugin_path => $state ) {
        if ( $state ) {
          require_once( $this->theme_path . '/plugins/' . $plugin_path );
        }
      }
    }

    /**
     * Determine if given plugin is active
     *
     * @param string $slug
     * @return bool
     */
    private function get_plugin_state( $slug ) {
      if ( isset( $this->plugin_states[$slug] ) ) {
        return $this->plugin_states[$slug];
      }
      return false;
    }

    /**
     * Retrieve active status for all plugins in given directory
     *
     * @return array
     */
    private function get_plugin_states() {
      $this->plugin_paths = $this->get_plugin_paths( $this->theme_path . '/plugins' );
      $states             = [];
      foreach ( $this->plugin_paths as $path ) {
        if ( isset( get_option( 'pie_plugin_states' )[$path] ) ) {
          $states[$path] = get_option( 'pie_plugin_states' )[$path];
        } else {
          $states[$path] = false;
        }
      }
    ...
    */
    ...
    /**
     * Update required plugins (activate/deactivate)
     */
      ...
      if ( isset( $_POST['plugin-activate'] ) || isset( $_POST['plugin-deactivate'] ) ) {
        if ( isset( $_POST['theme_plugin_state_change_nonce'] ) ) {
          if ( ! wp_verify_nonce( $_POST['theme_plugin_state_change_nonce'], 'theme_plugin_state_change_nonce' ) ) {
            ...
            $plugin                         = isset( $_POST['plugin-activate'] ) ? $_POST['plugin-activate'] : $_POST['plugin-deactivate'];
            $this->plugin_states[ $plugin ] = isset( $_POST['plugin-activate'] ) ? true : false;
            update_option( 'pie_plugin_states', $this->plugin_states );
    ...
    */
    public function plugins_callback() {
      ...
      $table = new PiePluginListTable( $this->plugin_paths, $this->plugin_states, $this->theme_path );
      $table->prepare_items();
      $table->display();
    }
